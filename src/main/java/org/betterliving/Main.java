...
public class Main {
	public static void main(String[] args) {
		...
		if (qsController.getAllQuestions().isEmpty()) {
			// --- TRUE / FALSE QUESTIONS (5 Points) ---
			qsController.addTrueFalse("Is carbon dioxide the only greenhouse gas driving climate change?", false, 5);
			qsController
			...
			qsController.addTrueFalse("Glaciers and ice sheets melting causes sea levels to fall.", false, 5);
			...
			qsController.addShortAnswer("What gas is released into the atmosphere primarily by burning fossil fuels?",
					"Carbon Dioxide", 10);
			qsController.addShortAnswer("What international treaty on climate change was adopted in 2015?",
					"Paris Agreement", 10);
			qsController.addShortAnswer(
					...
					10);
			qsController.addShortAnswer("What is the main unit used to measure carbon footprints?", "Metric Tons", 10);
			qsController.addShortAnswer("What acronym represents the UN panel that assesses climate change science?",
					"IPCC", 10);
			qsController.addShortAnswer(
			...
			qsController.addMultipleChoice("What does the 'cap and trade' system look to regulate?", "Carbon Emissions",
					15,
					Arrays.asList("Water Usage", "Deforestation", "Carbon Emissions", "Plastic Waste"));

			qsController.addMultipleChoice("Which country historically has emitted the most cumulative CO2?",
					"United States", 15,
		...
		// Launch GUI
		SwingUtilities.invokeLater(() -> new MainMenuView(qsController, lmController, true));
	}
}
